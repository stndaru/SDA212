...
public class lab5copy6 {
    ...
    static class AVLTree {
        ...
        {  
            ...
            {  
                ...
                if(limitLeft <= head.key && head.key <= limitRight){
                    ...
                    // out.println(head.key+"/"+head.type+"/"+head.candy+" ");
        ...
        public String getMaxDiff(int a, int b) {
            ...
            //     if(arr.get(i).get(keySeti).size() > 1 && tempDif == 0 && minTemp < keySeti){
            //         minTemp = keySeti;
            //         maxTemp = keySeti; 
            //     }
            //     // out.println("MINTEMP: " + minTemp + " MAXTEMP: " + maxTemp);
            //     // out.println("KEYSETI: " + keySeti);
            //     // out.println("ARRS: " + arr.size());

            //     for(int j = arr.size()-1; j > i; j--){
            //         int keySetj = (int) arr.get(j).keySet().toArray()[0];

            //         // out.println("KEYSETI: " + keySeti);
            //         // out.println("KEYSETJ: " + keySetj);

            //         if( keySetj - keySeti  >= tempDif
            //         && !(arr.get(i).get(keySeti).contains(arr.get(j).get(keySetj).toArray()[0])
            //         && arr.get(i).get(keySeti).size() == 1) && arr.get(j).get(keySetj).size() == 1){
