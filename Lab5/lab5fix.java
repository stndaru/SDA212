...
public class lab5fix {
    ...
    static class AVLTree {
        ...
        // and https://www.geeksforgeeks.org/floor-in-binary-search-tree-bst/?ref=lbp
        ...
        public String getMaxDiff(int a, int b) {
            ...
            int tempMin;
            int tempMax;
            int leftLimitOriginal = a;
            int rightLimitOriginal = b;
            ...
            if(minFloor == maxCeil){
                if(minSet < 2) return "-1 -1";
                Object[] types = minNode.candy.values().toArray();
                int tempType = (int) types[0];
                for(int i = 1; i < types.length; i++){
                    if((int) types[i] != tempType){
                        clear = true;
                    }
                }
            }
            
            if(minFloor < a || minFloor > b || maxCeil < a || maxCeil > b || minFloor > maxCeil) 
                if(clear) return "-1 -1";

            if(minFloor == maxCeil && !clear) return "-1 -1";

            // Force iterate minimum candy move upwards
            while(!clear){
                if(maxCeil - minFloor <= 0) break;
                if(maxCeil - minFloor > tempDif){
                    if(minNode.candy.size() == 1){
                        if((int) minNode.candy.values().toArray()[0] == (int) maxNode.candy.values().toArray()[0]){
                            a += 1;
                            floorMax = ceil(root, a);
                            minFloor = floorMax.key;
                            minSet = floorMax.candy.size();
                            minNode = floorMax;
                            continue;
                        ...
                        for(int i : minNode.candy.values()){
                            if(i != (int) maxNode.candy.values().toArray()[0]) clear = true;
                        }
                        for(int i : maxNode.candy.values()){
                            if(i != (int) minNode.candy.values().toArray()[0]) clear = true;
                        }
                        if(!clear){
                            a += 1;
                            floorMax = ceil(root, a);
